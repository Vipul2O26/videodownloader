...
import { createRenderer, getRequestDependencies, getPreloadLinks, getPrefetchLinks } from 'file:///workspaces/videodownloader/node_modules/vue-bundle-renderer/dist/runtime.mjs';
import { parseURL, withoutBase, joinURL, getQuery, withQuery, withTrailingSlash, decodePath, withLeadingSlash, withoutTrailingSlash, joinRelativeURL, encodePath } from 'file:///workspaces/videodownloader/node_modules/ufo/dist/index.mjs';
import { renderToString } from 'file:///workspaces/videodownloader/node_modules/vue/server-renderer/index.mjs';
import destr, { destr as destr$1 } from 'file:///workspaces/videodownloader/node_modules/destr/dist/index.mjs';
import { createHooks } from 'file:///workspaces/videodownloader/node_modules/hookable/dist/index.mjs';
import { createFetch, Headers as Headers$1 } from 'file:///workspaces/videodownloader/node_modules/ofetch/dist/node.mjs';
...
import { klona } from 'file:///workspaces/videodownloader/node_modules/klona/dist/index.mjs';
import defu, { defuFn } from 'file:///workspaces/videodownloader/node_modules/defu/dist/defu.mjs';
import { snakeCase } from 'file:///workspaces/videodownloader/node_modules/scule/dist/index.mjs';
import { getContext } from 'file:///workspaces/videodownloader/node_modules/unctx/dist/index.mjs';
import { toRouteMatcher, createRouter } from 'file:///workspaces/videodownloader/node_modules/radix3/dist/index.mjs';
import consola, { consola as consola$1 } from 'file:///workspaces/videodownloader/node_modules/consola/dist/index.mjs';
import { ErrorParser } from 'file:///workspaces/videodownloader/node_modules/youch-core/build/index.js';
import { Youch } from 'file:///workspaces/videodownloader/node_modules/youch/build/index.js';
import { SourceMapConsumer } from 'file:///workspaces/videodownloader/node_modules/source-map/source-map.js';
import { AsyncLocalStorage } from 'node:async_hooks';
import { stringify, uneval } from 'file:///workspaces/videodownloader/node_modules/devalue/index.js';
import { captureRawStackTrace, parseRawStackTrace } from 'file:///workspaces/videodownloader/node_modules/errx/dist/index.mjs';
import { isVNode, isRef, toValue } from 'file:///workspaces/videodownloader/node_modules/vue/index.mjs';
import _wH6JrtIxmaSoA8lCPWFnE9z4lQeXW6H5z3l5aymEQw from 'file:///workspaces/videodownloader/node_modules/@nuxt/vite-builder/dist/fix-stacktrace.mjs';
import { fileURLToPath, URL as URL$1 } from 'node:url';
import { dirname as dirname$1, resolve as resolve$1, basename } from 'file:///workspaces/videodownloader/node_modules/pathe/dist/index.mjs';
import { getIcons } from 'file:///workspaces/videodownloader/node_modules/@iconify/utils/lib/index.mjs';
import { collections } from 'file:///workspaces/videodownloader/.nuxt/nuxt-icon-server-bundle.mjs';
import { createHead as createHead$1, propsToString, renderSSRHead } from 'file:///workspaces/videodownloader/node_modules/unhead/dist/server.mjs';
import { DeprecationsPlugin, PromisesPlugin, TemplateParamsPlugin, AliasSortingPlugin } from 'file:///workspaces/videodownloader/node_modules/unhead/dist/plugins.mjs';
import { walkResolver } from 'file:///workspaces/videodownloader/node_modules/unhead/dist/utils.mjs';

const serverAssets = [{"baseName":"server","dir":"/workspaces/videodownloader/server/assets"}];
...
storage.mount('root', unstorage_47drivers_47fs({"driver":"fs","readOnly":true,"base":"/workspaces/videodownloader","watchOptions":{"ignored":[null]}}));
storage.mount('src', unstorage_47drivers_47fs({"driver":"fs","readOnly":true,"base":"/workspaces/videodownloader/server","watchOptions":{"ignored":[null]}}));
storage.mount('cache:nuxt:payload', cacheDriver({"base":"/workspaces/videodownloader/.nuxt/cache/nuxt/payload"}));
storage.mount('build', unstorage_47drivers_47fs({"driver":"fs","readOnly":false,"base":"/workspaces/videodownloader/.nuxt"}));
storage.mount('cache', unstorage_47drivers_47fs({"driver":"fs","readOnly":false,"base":"/workspaces/videodownloader/.nuxt/cache"}));
storage.mount('data', unstorage_47drivers_47fs({"driver":"fs","base":"/workspaces/videodownloader/.data/kv"}));
...
const rootDir = "/workspaces/videodownloader";

const appHead = {"meta":[{"charset":"utf-8"},{"name":"viewport","content":"width=device-width, initial-scale=1"},{"name":"description","content":"Direct media downloader with validation, queueing, and secure media handling."}],"link":[],"style":[],"script":[],"noscript":[],"title":"Video Downloader"};
...
const _iZbv4cqXNDSXz969xocRzl1J4YJiKgBvJqh4J150i0 = (nitroApp) => {
...
const _zu0qtNrwDxQaGsXVx1qpnYIEHyMoO0kwKnU26bF6oxw = (function(nitro) {
...
const plugins = [
  _iZbv4cqXNDSXz969xocRzl1J4YJiKgBvJqh4J150i0,
_zu0qtNrwDxQaGsXVx1qpnYIEHyMoO0kwKnU26bF6oxw,
...
const _qf7BW2 = eventHandler((event) => {
...
function getHostname(url) {
  try {
    return new URL$1(url).hostname.toLowerCase();
  } catch {
    return null;
  }
}
function validateUrl(rawUrl) {
  ...
  try {
    const parsed = new URL$1(trimmed);
...
function assertSizeLimit(fileSizeBytes, byteLimit = 50 * 1024 * 1024) {
...
async function withTimeout(promise, timeoutMs, message = "Request timeout exceeded") {
  let timer;
...
function enforceRateLimit(key, maxRequests = 25, windowMs = 6e4) {
  var _a;
  const now = Date.now();
  const entries = (_a = rateLimiter.get(key)) != null ? _a : [];
...
const _NL986R = defineEventHandler((event) => {
  var _a;
  ...
  const remoteAddress = Array.isArray(forwarded) ? forwarded[0] : (_a = forwarded != null ? forwarded : event.node.req.socket.remoteAddress) != null ? _a : "unknown-client";
  if (!enforceRateLimit(remoteAddress, 20, 6e4)) {
    throw createError({
      statusCode: 429,
...
const TEMP_DIRECTORY = path.resolve(process.cwd(), ".tmp");
async function ensureTemporaryDirectory() {
  await promises.mkdir(TEMP_DIRECTORY, { recursive: true });
  return TEMP_DIRECTORY;
}
async function writeTemporaryFile(content, extension = "bin", prefix = "media") {
  await ensureTemporaryDirectory();
  const fileName = `${prefix}-${v4()}.${extension.replace(/^\./, "")}`;
  const targetPath = path.join(TEMP_DIRECTORY, fileName);
  await promises.writeFile(targetPath, Buffer.from(content));
  return targetPath;
}
async function cleanupTemporaryFiles(maxAgeMs = 60 * 60 * 1e3) {
  await ensureTemporaryDirectory();
  const files = await promises.readdir(TEMP_DIRECTORY);
  const now = Date.now();
  await Promise.all(
    files.map(async (fileName) => {
      const filePath = path.join(TEMP_DIRECTORY, fileName);
      const stats = await promises.stat(filePath).catch(() => null);
      if (!stats || !stats.isFile()) {
        return;
      }
      if (now - stats.mtimeMs > maxAgeMs) {
        await promises.unlink(filePath).catch(() => void 0);
      }
    })
  );
}
...
class BackgroundQueue {
  ...
  async list() {
    if (this.redis) {
      ...
      return items.map((entry) => JSON.parse(entry));
    }
    return [...memoryQueue];
  }
...
async function direct_media_download(url, options = {}) {
  return await download_authorized_media(url, options);
}

const warnOnceSet = /* @__PURE__ */ new Set();
const DEFAULT_ENDPOINT = "https://api.iconify.design";
const _aWSryn = defineCachedEventHandler(async (event) => {
...
const handlers = [
  { route: '', handler: _qf7BW2, lazy: false, middleware: true, method: undefined },
  { route: '', handler: _NL986R, lazy: false, middleware: true, method: undefined },
  ...
  { route: '/_fonts/**', handler: _lazy_3K2MZW, lazy: true, middleware: false, method: undefined },
  { route: '/**', handler: _lazy_3K2MZW, lazy: true, middleware: false, method: undefined }
];
...
const mediaInfo_post$1 = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.defineProperty({
  __proto__: null,
  default: mediaInfo_post
}, Symbol.toStringTag, { value: 'Module' }));
...
const validateUrl_post$1 = /*#__PURE__*/Object.freeze(/*#__PURE__*/Object.defineProperty({
  __proto__: null,
  default: validateUrl_post
}, Symbol.toStringTag, { value: 'Module' }));
